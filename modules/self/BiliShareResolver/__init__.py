...
async def get_video_info(video_id: str) -> dict:
    ...
    if video_id[:2].lower() == "av":
        async with session.get(
            f"http://api.bilibili.com/x/web-interface/view?aid={video_id[2:]}"
        ) as resp:
...
template = (Path(__file__).parent / "video.html").read_text(encoding="utf-8")
...
async def gen_img(data: VideoInfo) -> bytes:
    ...
    async with session.get(
        f"https://api.bilibili.com/x/relation/stat?vmid={data.up_mid}"
    ) as resp:
    ...
    return await template2img(
        template,
        {
            ...
            "publish_time": time.strftime(
                "%Y/%m/%d %p %I:%M:%S", time.localtime(data.upload_timestamp)
            ),
        ...
        page_option={"viewport": {"width": 960, "height": 10}},
    )
